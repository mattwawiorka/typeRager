...
const commands = {
	...
	getPlayerWPM: (msg) => {
		...
		con.query("SELECT username FROM users WHERE discordUser =\""+user+"\"", (err, result) => {
			if (err) throw err;
			if (result.length < 1) {
				msg.channel.send(user + " has not been added, they must add themselves");
				return;
			}
			try {
	...
	listCommands: (msg) => {
		msg.reply(
			`My commands are:
			\n!race - gets a racetrack for users to join
			...
			\n!add - adds a user as a racer, provide your TypeRacer username`
		);
	}
}
